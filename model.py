...
class BrainRSNADataset(Dataset):
    ...
    def _prepare_biggest_images(self):
        ...
        big_image_indexes = {}
        if (f"big_image_indexes_{self.folder}_{self.ds_type}_{self.type}.pkl" in os.listdir("indices/"))\
            and (self.do_load) :
            print("Loading the best images indexes for all the cases...")
            big_image_indexes = joblib.load(f"indices/big_image_indexes_{self.folder}_{self.ds_type}_{self.type}.pkl")
            ...
            joblib.dump(big_image_indexes, f"indices/big_image_indexes_{self.folder}_{self.ds_type}_{self.type}.pkl")
            return big_image_indexes
...
df_test_ds = BrainRSNADataset(data=df_test, mri_type="FLAIR",ds_type="val" , is_train= False)

df_test_dl =  torch.utils.data.DataLoader(
    df_test_ds,
...
with torch.no_grad():
        ...
        true_labels = []
        case_ids = []
        epoch_iterator_val = tqdm(df_test_dl)
